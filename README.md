...
- 发给模型的系统提示词**全部使用英文**（[worker/prompts.ts](worker/prompts.ts)）：部分模型对中文指令理解不佳，且本应用主要处理英文内容。
- 提示词会**注入检测到的原文语言**，并明确「严禁翻译 / 同语言输出」，专有名词、数字、日期、单位、URL、代码要求原样保留；`changes[].reason` 仍按「说明语言」设置输出中文或英文。
- 润色与校对结果有一道**语言一致性校验**：若模型把中文润色成了英文（或反之），会自动带上语言指令重试一次；仍不一致则退回原文，避免把译文当润色结果返回。
- 小模型偶尔输出轻微破损的 JSON 时，会先尝试强制重试严格 JSON，再尽力从文本中提取 `result` 字段，避免把 JSON 原文当结果显示。
